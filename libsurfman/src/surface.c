...
static int compar_pfns(const void *a, const void *b)
{
...
/*
 * Pin cache-attributes of a pfns range from a guest.
 * On guests without auto-translation (full PV), this will return an error.
 * While it may seem overkill to qsort() the pfns, there are no guaranty that
 * this is a contiguous range of pfns. Most likely it is not, virtual
                                 ...
                                 unsigned int cacheattr)
{
  ...
  for (i = 0; i < surface->page_count; ++i)
    pfns[i] = p->u.pfn_arr.arr[i];

  qsort (pfns, surface->page_count, sizeof (pfns[0]), &compar_pfns);
  for (i = 0; i < surface->page_count; i = j)
    {
      ...
      if (xc_hvm_pin_memory_cacheattr (surface->pages_domid,
                                       pfns[i], pfns[j - 1],
                                       XC_MAP_CACHEATTR_WC))
        surfman_warning ("Failed to change cache attributes for dom%u %#lx-%#lx.",
                         surface->pages_domid, pfns[i], pfns[j - 1]);
    }
  free (pfns);
